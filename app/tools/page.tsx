    ...
    <main className="min-h-screen bg-[#f4f2eb] pt-32 pb-32 px-4 sm:px-10 text-stone-900">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="border-b border-stone-300 pb-6 mb-8">
          <span className="font-mono text-xs text-[#1d3a28] font-bold uppercase">// KATALOG UTAMA ALAT SEWA</span>
          <h1 className="text-4xl font-extrabold tracking-tight mt-1">Sewa Alat Wisata Ternate</h1>
        </div>

        {/* Multi-Filter: Kategori Umum & Toko Pemilik */}
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between">
              ...
              {["Semua", "Camping", "Bahari", "Fotografi", "Hiking"].map((cat) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl uppercase tracking-wider transition-colors cursor-pointer border shrink-0 ${
                    selectedCategory === cat ? "bg-[#1d3a28] text-white border-[#1d3a28]" : "bg-white text-stone-700 border-stone-300"
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 bg-white border border-stone-300 rounded-xl text-xs w-full sm:w-64 focus:outline-none focus:border-[#1d3a28]"
              />
            </div>
          </div>

          {/* Filter Berdasarkan Toko Pemilik Barang */}
          <div className="flex items-center gap-2 overflow-x-auto text-xs">
            ...
            {ownerList.map((owner) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-[11px] font-medium transition-colors cursor-pointer border shrink-0 ${
                  selectedOwner === owner ? "bg-stone-800 text-white border-stone-800" : "bg-white text-stone-600 border-stone-200"
                }`}
              >
                {owner}
              </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filtered.map((tool) => {
            ...
            return (
              <div key={tool.id} className="bg-white border border-stone-300 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow text-center group">
                <div 
                  onClick={() => setPreviewData({ type: "tool", id: tool.id, name: tool.name, price: tool.price, img: tool.img, categoryOrLang: tool.category, rating: tool.rating })}
                  className="relative w-28 h-28 rounded-xl overflow-hidden mx-auto mb-3 border border-stone-200 bg-stone-100 shadow-inner cursor-pointer"
                >
                ...
                <h3 
                  onClick={() => setPreviewData({ type: "tool", id: tool.id, name: tool.name, price: tool.price, img: tool.img, categoryOrLang: tool.category, rating: tool.rating })}
                  className="font-bold text-stone-900 text-lg mb-0.5 cursor-pointer hover:text-[#1d3a28]"
                >
                  {tool.name}
                </h3>
                
                <p className="text-[#c5922e] text-xs font-bold mb-1 flex items-center justify-center gap-1">
                  <Store className="w-3.5 h-3.5" /> Pemilik: {tool.ownerName} ({tool.location})
                </p>
                ...
                <div className="pt-2 border-t border-stone-200 flex items-center justify-between text-xs text-stone-600 mb-4 font-sans">
                  <span>Stok: <strong>{tool.stock} Unit</strong> ({tool.rentCount}x Disewa)</span>
                  <span className="text-[#1d3a28] font-bold flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> Tersedia</span>
                </div>

                <button
                  onClick={() => toggleTool({ id: tool.id, name: tool.name, price: tool.price, img: tool.img })}
                  className={`w-full py-2.5 rounded-xl text-xs uppercase tracking-wider font-bold transition-colors cursor-pointer flex items-center justify-center gap-2 ${
                    isSelected ? "bg-[#1d3a28] text-white" : "bg-stone-900 text-white hover:bg-[#1d3a28]"
                  }`}
                >
