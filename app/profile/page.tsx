    ...
    return (
      <main className="min-h-screen bg-[#f4f2eb] pt-32 px-4 text-center text-stone-900">
        ...
        <div className="max-w-md mx-auto bg-white p-8 border border-stone-300 rounded-none mt-12 space-y-4">
          ...
          <button onClick={() => router.push("/")} className="bg-[#1d3a28] text-white px-6 py-2.5 rounded-none font-mono text-xs uppercase font-bold cursor-pointer">Kembali</button>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#f4f2eb] pt-32 pb-32 px-4 sm:px-10 text-stone-900 font-sans">
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="border-b border-stone-300 pb-6 flex justify-between items-end">
          ...
          <button onClick={() => setIsLogoutModalOpen(true)} className="px-5 py-2.5 bg-rose-100 text-rose-700 hover:bg-rose-200 rounded-none text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 cursor-pointer border border-rose-200">
            <LogOut className="w-4 h-4" /> Keluar
          </button>
        </div>

        {myDisputes.length > 0 && (
          <div className="bg-rose-100 border border-rose-300 rounded-none p-6 flex items-start gap-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-8 space-y-6">
            {user.role === "customer" && (
              <>
                {/* FIX: Kategorisasi Riwayat Pesanan */}
                <div className="bg-white border border-stone-300 rounded-none p-6 space-y-6">
                  ...
                  <div>
                    ...
                    {activeOrders.length === 0 ? <p className="text-xs text-stone-500 font-mono">Tidak ada pesanan aktif.</p> : activeOrders.map((ord) => (
                      <div key={ord.orderId} className="p-4 bg-[#f4f2eb] border border-stone-300 rounded-none space-y-2 text-xs mb-3">
                        ...
                        <div className="flex justify-between items-center pt-2 gap-2">
                          ...
                          <div className="flex gap-2">
                            <Link href={`/chat/${ord.orderId}`} className="px-3 py-1.5 bg-white border border-stone-800 text-stone-900 text-[11px] font-bold uppercase rounded-none flex items-center gap-1"><MessageSquare className="w-3 h-3" /> Chat Toko</Link>
                            <Link href="/ticket/TRX-GAMTARA-7890" className="px-3 py-1.5 bg-[#1d3a28] text-white text-[11px] font-bold uppercase rounded-none flex items-center gap-1"><Ticket className="w-3 h-3" /> Tiket QR</Link>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div>
                    ...
                    {completedOrders.length === 0 ? <p className="text-xs text-stone-500 font-mono">Tidak ada pesanan selesai.</p> : completedOrders.map((ord) => (
                      <div key={ord.orderId} className="p-4 bg-stone-100 border border-stone-200 rounded-none space-y-2 text-xs mb-3 opacity-75">
                  ...
                  {disputeOrders.length > 0 && (
                    <div>
                      ...
                      {disputeOrders.map((ord) => (
                        <div key={ord.orderId} className="p-4 bg-rose-50 border border-rose-200 rounded-none space-y-2 text-xs mb-3">
                ...
                <div className="bg-white border border-stone-300 rounded-none p-6 space-y-4">
                  ...
                  {guideRequests.length === 0 ? <p className="text-xs text-stone-500 font-mono py-2">Belum ada permintaan pemandu dikirim.</p> : guideRequests.map((req) => (
                    <div key={req.id} className="p-4 bg-[#f4f2eb] border border-stone-300 rounded-none flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-xs">
                      ...
                      <div className="flex gap-2">
                        {req.status === "DISETUJUI" && <button onClick={() => setPayingReqId(req.id)} className="px-4 py-2 bg-[#1d3a28] text-white text-xs font-bold uppercase font-mono rounded-none flex items-center gap-1.5 cursor-pointer"><QrCode className="w-3.5 h-3.5 text-[#c5922e]" /> Bayar QRIS Sekarang</button>}
                        {req.status === "LUNAS" && <Link href={`/chat/${req.id}`} className="px-4 py-2 bg-[#1d3a28] hover:bg-[#152a1b] text-white text-xs font-bold uppercase rounded-none flex items-center gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> Buka Room Chat</Link>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {(user.role === "pemilik" || user.role === "pemandu") && (
              <div className="bg-white border border-stone-300 rounded-none p-8 text-center space-y-4">
                ...
                <Link href={user.role === "pemilik" ? "/vendor/pemilik" : "/vendor/pemandu"} className="inline-block px-6 py-3 bg-[#1d3a28] text-white text-xs font-bold font-mono uppercase tracking-wider rounded-none">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-sm">
            <div className="bg-[#f4f2eb] p-8 rounded-none max-w-sm w-full text-center border border-stone-300 shadow-2xl space-y-6">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setIsLogoutModalOpen(false)} className="flex-1 py-3 bg-white border border-stone-300 text-stone-800 font-bold rounded-none text-xs uppercase cursor-pointer hover:bg-stone-100">Batal</button>
                <button onClick={handleConfirmLogout} className="flex-1 py-3 bg-rose-600 text-white font-bold rounded-none text-xs uppercase cursor-pointer hover:bg-rose-700">Ya, Keluar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {payingReqId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-sm">
            <div className="bg-[#f4f2eb] p-8 rounded-none max-w-sm w-full text-center border border-stone-300 shadow-2xl space-y-6">
              ...
              <button onClick={handleConfirmPayGuide} className="w-full py-3 bg-[#1d3a28] hover:bg-[#152a1b] text-white rounded-none font-mono text-xs uppercase font-bold tracking-wider cursor-pointer shadow-md">
