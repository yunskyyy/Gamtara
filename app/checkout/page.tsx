...
export default function CheckoutPage() {
  ...
  // FIX: Kalkulasi Hari Sewa (Selisih Hari + 1)
  const totalDays = React.useMemo(() => {
    const start = new Date(startDate);
    const end = new Date(endDate);
    const diffTime = Math.abs(end.getTime() - start.getTime());
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
    return diffDays + 1; // Ditambah 1 agar dihitung hari penuh
  }, [startDate, endDate]);
                  ...
                  <div key={vendorName} className={`bg-white border rounded-none p-6 ${isVendorPaid ? "border-emerald-500 bg-emerald-50/50" : "border-stone-300"}`}>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-[#f9f8f3] p-5 border border-stone-200 rounded-none flex flex-col justify-between">
                        ...
                        {!isVendorPaid ? (
                          <button onClick={() => setPayingVendor(vendorName)} className="w-full py-3 bg-[#1d3a28] hover:bg-[#152a1b] text-white font-bold uppercase tracking-wider rounded-none transition-colors cursor-pointer flex items-center justify-center gap-2">
