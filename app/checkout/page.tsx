...
export default function CheckoutPage() {
  ...
  const { user } = useAuth();
  const { selectedTools, clearBooking } = useBooking();
  
  const today = new Date().toISOString().split('T')[0];
  const [startDate, setStartDate] = React.useState(today);
  const [endDate, setEndDate] = React.useState(today);
  
  // State untuk menyimpan opsi pengiriman per Toko
  ...
  // Grouping Keranjang Berdasarkan Toko (Vendor)
  ...
  const handleSimulatePayment = (vendorName: string) => {
    ...
    // Jika semua toko sudah dibayar, redirect ke profil
            ...
            return (
              <div key={vendorName} className={`bg-white border rounded-sm p-6 shadow-sm ${isPaid ? "border-emerald-500 bg-emerald-50/50" : "border-stone-300"}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Kiri: Rincian Barang & Opsi Pengiriman */}
        ...
        {payingVendor && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-sm">
            <div className="bg-[#f4f2eb] p-8 rounded-sm max-w-sm w-full text-center border border-stone-300 shadow-2xl space-y-6">
              ...
              <button onClick={() => handleSimulatePayment(payingVendor)} className="w-full py-3 bg-[#1d3a28] hover:bg-[#152a1b] text-white rounded-sm font-mono text-xs uppercase tracking-wider font-bold cursor-pointer transition-all flex items-center justify-center gap-2">
