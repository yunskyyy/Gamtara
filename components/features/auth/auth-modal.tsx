...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      const success = register({ name, email, phone, origin, address, gender, role, password });
      if (!success) {
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-[#f4f2eb] text-stone-900 border border-stone-300 rounded-2xl p-6 sm:p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 text-xs">
            ...
            <div>
              ...
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minimal 6 karakter" className="w-full px-3.5 py-2 rounded-xl bg-white border border-stone-300 focus:outline-none focus:border-[#1d3a28]" />
            </div>

            {!isLogin && (
              <div>
                <label className="block font-bold text-stone-700 mb-1">Konfirmasi Kata Sandi</label>
                <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Ulangi kata sandi" className="w-full px-3.5 py-2 rounded-xl bg-white border border-stone-300 focus:outline-none focus:border-[#1d3a28]" />
              </div>
            )}
