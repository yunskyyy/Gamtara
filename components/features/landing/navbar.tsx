    ...
    <>
      <motion.header className="fixed top-0 left-0 right-0 z-40 flex flex-col items-center pt-4 px-4 pointer-events-none">
        <nav className="pointer-events-auto flex items-center justify-between w-full max-w-5xl px-4 sm:px-8 py-1 bg-[#f4f2eb]/95 backdrop-blur-md border border-stone-300/80 rounded-sm shadow-md text-stone-900">
          <Link href="/" className="flex items-center cursor-pointer py-0.5"><Logo variant="full" height={48} /></Link>
          ...
          <ul className="hidden md:flex items-center gap-8 text-xs font-bold tracking-wider text-stone-700 uppercase">
            ...
            {/* LOGIKA RBAC: Hanya Wisatawan/Tamu yang bisa melihat Katalog */}
            {isCustomerOrGuest && (
              <>
            ...
            {user && <li><Link href="/profile" className="hover:text-[#1d3a28] transition-colors flex items-center gap-1 font-extrabold text-[#1d3a28]"><User className="w-3.5 h-3.5" /> Profil Saya</Link></li>}
            {user?.role === "admin" && <li><Link href="/admin/dashboard" className="text-rose-700 font-extrabold hover:underline">Admin</Link></li>}
            {user?.role === "pemilik" && <li><Link href="/vendor/pemilik" className="text-[#1d3a28] font-extrabold hover:underline">Dashboard Pemilik</Link></li>}
            {user?.role === "pemandu" && <li><Link href="/vendor/pemandu" className="text-[#1d3a28] font-extrabold hover:underline">Dashboard Pemandu</Link></li>}
          </ul>
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="flex items-center gap-2">
                ...
                <button onClick={logout} className="p-2 bg-stone-200 hover:bg-stone-300 rounded-sm cursor-pointer text-stone-700" title="Keluar"><LogOut className="w-4 h-4" /></button>
              </div>
            ) : (
              <button onClick={() => setIsAuthOpen(true)} className="bg-[#1d3a28] hover:bg-[#152a1b] text-stone-100 px-5 py-2 rounded-sm text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm">Masuk</button>
            )}
