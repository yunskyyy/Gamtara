  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-sm">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-2xl bg-[#f4f2eb] text-stone-900 border border-stone-300 rounded-2xl overflow-hidden shadow-2xl p-6 sm:p-8">
          <button onClick={onClose} className="absolute top-4 right-4 z-20 p-2 rounded-xl bg-stone-200 text-stone-800 hover:bg-stone-300 transition-colors cursor-pointer">
            <X className="w-4 h-4" />
          </button>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
            <div className="h-64 sm:h-72 w-full rounded-xl overflow-hidden bg-stone-200 border border-stone-300 shadow-sm">
              <img src={data.img} alt={data.name} className={`w-full h-full object-cover ${!isTool ? "object-top" : ""}`} />
            </div>
            ...
            <div className="flex flex-col justify-between h-full py-2">
              <div>
                ...
                <p className="text-stone-600 text-xs sm:text-sm leading-relaxed mb-4">{data.desc || "Deskripsi lengkap rincian layanan."}</p>
