  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/90 backdrop-blur-sm">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="relative w-full max-w-4xl bg-[#f4f2eb] text-stone-900 shadow-2xl flex flex-col sm:flex-row overflow-hidden border border-stone-300 rounded-sm">
          <button onClick={onClose} className="absolute top-4 right-4 z-20 flex items-center gap-1 text-stone-500 hover:text-stone-900 transition-colors cursor-pointer font-mono text-[10px] font-bold tracking-widest uppercase">
          ...
          <div className="w-full sm:w-1/2 h-64 sm:h-[480px] bg-stone-200 border-b sm:border-b-0 sm:border-r border-stone-300">
            <img src={data.img} alt={data.name} className={`w-full h-full object-cover grayscale-[10%] ${!isTool ? "object-top" : ""}`} />
          </div>
          ...
          <div className="w-full sm:w-1/2 p-8 sm:p-12 flex flex-col justify-center bg-[#f9f8f3]">
            <div>
              ...
              <h2 className="text-3xl sm:text-4xl font-extrabold text-stone-900 mb-4 leading-tight tracking-tight">{data.name}</h2>
              <p className="text-stone-600 text-sm leading-relaxed mb-8 font-light">{data.desc || "Deskripsi lengkap rincian layanan."}</p>
              <div className="border-t border-stone-300 pt-4 mb-8">
                ...
                <p className="text-[#1d3a28] font-black text-2xl font-mono">Rp {data.price.toLocaleString("id-ID")}</p>
              </div>
            </div>

            {isTool && (
              <button
                onClick={() => toggleTool({ id: data.id, name: data.name, price: data.price, ownerName: data.ownerName || "Toko Gamalama Outdoor", img: data.img, vendorId: data.vendorId || "v-default", lat: 0, lng: 0 })}
